...
def test_place_person_moves_face_to_reference_box(monkeypatch: Any) -> None:
    """AI가 얼굴을 작게·치우치게 그려도 카드에선 기준 크기·위치로 맞춰져야 한다."""
    from PIL import ImageDraw

    from app.services import id_card_renderer as r

    # 1024x1536 회색 사진, 얼굴(빨간 사각형)은 작고(200px) 중앙에서 벗어나 있다.
    src = Image.new("RGBA", (1024, 1536), (210, 210, 210, 255))
    ImageDraw.Draw(src).rectangle((350, 200, 549, 399), fill=(255, 0, 0, 255))
    monkeypatch.setattr(r, "detect_face", lambda img: (350, 200, 200, 200))

    size = (r.CARD_W, round(r.CARD_H * r.PHOTO_BOTTOM))
    out = r._place_person(src, size)
    red = [
        (x, y)
        for y in range(0, size[1], 4)
        for x in range(0, size[0], 4)
        if out.getpixel((x, y))[:3] == (255, 0, 0)
    ]
    xs, ys = [p[0] for p in red], [p[1] for p in red]

    assert out.size == size
    assert abs((max(xs) - min(xs)) - r.FACE_W * r.CARD_W) < 12  # 기준 폭으로 확대
    assert abs((min(xs) + max(xs)) / 2 - r.FACE_CX * r.CARD_W) < 12  # 가로 중앙
    assert abs(min(ys) - r.FACE_TOP * r.CARD_H) < 12  # 기준 높이


def test_place_person_never_leaves_gaps(monkeypatch: Any) -> None:
    """얼굴이 기준보다 커서 줄여야 해도 영역을 다 덮는 선에서 멈춘다(빈 공간 없음)."""
    from app.services import id_card_renderer as r

    src = Image.new("RGBA", (1024, 1536), (210, 210, 210, 255))
    monkeypatch.setattr(r, "detect_face", lambda img: (100, 100, 800, 800))

    size = (r.CARD_W, round(r.CARD_H * r.PHOTO_BOTTOM))
    out = r._place_person(src, size)

    assert out.getchannel("A").getextrema() == (255, 255)  # 투명(빈) 픽셀 없음
