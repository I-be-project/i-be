...
> 본 문서는 나Be한마당 페르소나 카드(진로 내비게이터) 프로젝트의 **백엔드 시스템 설계 종합 문서**다. 프로젝트 비전과 UX 의도는 [`../notes/2026-03-19-프로젝트-비전.md`](../notes/2026-03-19-프로젝트-비전.md)를 참고한다. 본 문서는 그 비전을 **현실의 시스템 컴포넌트로 어떻게 구현할 것인가**를 다룬다.

| 항목 | 값 |
|---|---|
| 최초 작성 | 2026-05-31 |
| 적용 대상 | `backend/` |
| 요약본 | [`architecture.md`](architecture.md) — 먼저 읽으면 이해가 빠르다 |

> **⚠️ 이 문서는 설계 의도 중심이다.** 2026-05-31 초안 이후 구현이 여러 곳에서
> 갈라졌다. 아래는 **본문과 현재 구현이 다른 지점**이며, 각 절에도 표시해뒀다.
> 구현 현황을 빠르게 보려면 [`architecture.md`](architecture.md) §12를 본다.
>
> | 본문 서술 | 현재 |
> |---|---|
> | schema 4개 (`rewards` 포함) | **3개** — 부스는 `ops.booths`, 방문은 `ops.booth_visits` |
> | Supabase Storage, `photos/`·`cards/` | **S3 단일 버킷**, `uploads/`·`ai-images/`·`cards/` |
> | 사진은 카드 생성 후 폐기 | **영구 보관** (§10.1) |
> | 학생 토큰 6시간 | **12시간** |
> | CORS 프론트 도메인만 | **전체 개방(`*`)** |
> | 잡 큐·워커·카드 발급 | **🔴 미구현** — `cards.py` 전부 `NotImplementedError` |
> | 감사 로그 | 🔴 미구현 |
>
> 반영된 것: 계정 종류(`kind` = student/guest/test), 부스 QR 방문 인증,
> 운영진 콘솔, 운영 시크릿 기동 가드.
...
- 이 문서는 **왜 이렇게 설계했는가**를 담는다. 지금 무엇이 동작하는가는
  [`architecture.md`](architecture.md)를 본다.
- 결정 기록(ADR)은 아직 별도 폴더로 분리하지 않았다. 결정이 3개 이상 쌓이면
  `docs/decisions/`로 승격한다 → [`../README.md`](../README.md) "폴더 승격 규칙"
...
7. **개인정보 보호 정책 강제** — 사진 영구 보관(동의 기반), 학생 삭제 요청 처리, 권한 격리
...
| 객체 스토리지 | **AWS S3** (단일 비공개 버킷) | 초안은 Supabase Storage였으나 S3로 교체. 노출은 presigned URL만 |
...
└──────────────┬───────────────────────────┬─────────────────────┘
               ▼                           ▼
   ┌──────────────────────┐    ┌────────────────────────────┐
   │ Supabase Postgres    │    │ AWS S3 (단일 비공개 버킷)   │
   │                      │    │  uploads/    원본 사진      │
   │  schemas:            │    │  ai-images/  AI 인물        │
   │   pii                │    │  cards/      최종 카드      │
   │   generated          │    └────────────────────────────┘
   │   ops                │    ┌────────────────────────────┐
   └──────────────────────┘    │ OpenRouter                 │
...
- DB 접근은 모두 Repository 레이어를 통하고, 외부 시스템(OpenRouter, S3)은 Adapter로 추상화.
  이 추상화 덕에 스토리지를 Supabase Storage → S3로 바꾸는 변경이 국소적으로 끝났다.
...
물리적으로 **Postgres 1개**, 논리적으로 **schema 3개**로 책임과 권한을 나눈다.

| Schema | 실제 테이블 | 책임 |
|---|---|---|
| `pii` | `students` | 학생 원본(이름·학교·학년·반·번호·성별·사진키·`kind`) |
| `generated` | `sessions` `answers` `personas` `cards` | 설문·답변·페르소나·카드 |
| `ops` | `settings` `booths` `booth_visits` | 운영 설정·부스·방문 기록 |

> 초안의 `rewards` 스키마는 만들지 않았다. **부스는 `ops.booths`, 방문 기록은
> `ops.booth_visits`** 에 있다. 아래 §6.2의 `rewards.*` 서술은 설계 의도이며
> 현재 스키마와 다르다.
...
**pii 스키마**
- `students` — 학생 1명을 식별하는 행. 이름·학교·학년·반·번호·성별·동의 플래그·사진 키·`kind`를 보관. 사진은 영구 보관하며, **학생의 삭제 요청 시에만** 파일과 키를 지운다.
...
- 이미지·사진 바이너리: **AWS S3 단일 비공개 버킷**, DB엔 키만 보관. 접근은 presigned URL.
  - `uploads/` — 학생 얼굴 원본. **영구 보관** (자동 폐기하지 않는다).
  - `ai-images/` — AI가 생성한 인물 이미지. 영구 보관.
  - `cards/` — 최종 카드 이미지. 영구 보관.

> 초안은 Supabase Storage에 `photos/`·`cards/` 두 버킷을 쓰고 사진을 카드 생성 후
> 폐기하는 안이었다. 현재는 **S3 단일 버킷 + 프리픽스 3종**이고 **사진은 영구 보관**이다.
> 보존 정책은 §10.1 참고.
...
| 종류 | 용도 | 만료 | claim |
|---|---|---|---|
| **학생 세션 토큰** | 질문 진행 중 API 호출 | **12시간** | `sub=student_id`, `kind=student`, `session_id` |
...
> **이 절 전체가 설계 의도이며 구현이 아니다.** `ops.jobs` 마이그레이션이 없고
> `backend/app/routers/cards.py`의 엔드포인트 4개는 모두 `NotImplementedError`다.
> 필요 작업량은 [`../notes/2026-08-08-행사-부하-용량-점검.md`](../notes/2026-08-08-행사-부하-용량-점검.md) §2·§7.2 참고 (3~5일 추산).
...
| 데이터 | 보존 기간 | 위치 | 비고 |
|---|---|---|---|
| **얼굴 사진 원본** | **영구** | S3 `uploads/` | **자동 폐기하지 않는다** |
| AI 생성 인물·카드 | **영구** | S3 `ai-images/`·`cards/` | 본인 페이지에서 조회 |
| 이름/학교/학년/반/번호 | **영구** | `pii` | 학생 요청 시 즉시 삭제 |
| 설문 응답 | **영구** | `generated` | 통계 활용은 별도 동의 |
| 페르소나/카드 메타 | **영구** | `generated` | |
| 부스 방문 | **영구** | `ops.booth_visits` | 운영 통계 |

> **사진은 영구 보관이 확정 방침이다.** 초안에 있던 "카드 생성 후 즉시 폐기,
> 동의 시 30일 보관"은 폐기됐다. 대신 **가입 동의문에 영구 보관 및 이용 목적을
> 명시**하고, 만 14세 미만은 법정대리인 동의(가정통신문)를 받는다.
> 삭제 요청 절차(§10.3)는 그대로 유지한다.

### 10.2 동의 항목

1. **[필수]** 카드 생성을 위한 이름·사진·응답 수집·처리
2. **[필수]** AI 분석 처리 (OpenRouter로 전송됨)
3. **[필수]** **사진 원본을 포함한 결과의 영구 보관** 및 본인 조회 제공
4. **[선택]** 익명화된 응답 데이터를 통계·연구 목적으로 활용

만 14세 미만은 법정대리인 동의가 필요하다.
...
- HTTPS 강제
- **CORS는 현재 전체 개방(`*`)이다.** 관리자 API를 외부 기관에 전달하려고 연 것으로,
  설계 초안의 "프론트 도메인만 화이트리스트"와 다르다.
...
- 행사 후: 인스턴스 다운그레이드 (**사진은 폐기하지 않는다** — §10.1)
...
| # | 항목 | 상태 |
|---|---|---|
| O-1 | AI 모델 조합 | **결정** — `AI_MODEL=openai/gpt-5-mini`, 단계별 오버라이드 가능 |
| O-2 | 이미지 모델 | **결정** — `AI_IMAGE_MODEL=google/gemini-3.1-flash-image-preview` |
| O-3 | 생성형 질문(Q7~9) 저장 형식 | **결정** — 질문 문장까지 `generated.answers`에 저장 |
| O-4 | 부스-페르소나 추천 매핑 알고리즘 | 미결정 |
| O-5 | 공유 토큰 만료시간 | **결정** — 24h (`share_link_ttl_hours`) |
| O-6 | 워커 프로세스 분리 임계점 | 미결정. 부하 점검은 **분리를 P0로 권고** |

---

## 15. 남은 작업

초기 스캐폴딩·마이그레이션·프롬프트는 완료됐다. 현재 남은 것:

1. **카드 파이프라인 구현** (§9) — `ops.jobs` + `CardRepository` + 워커 + `/api/cards/*`
2. 부하 테스트 시나리오 실행
3. 관측성(§13) — 현재 구조화 로깅만 있고 메트릭·알림은 없다
4. ADR 분리 — 결정이 3개 쌓이면 `docs/decisions/`로 승격

행사 전 필수 작업 전체 목록은
[`../notes/2026-08-08-행사-부하-용량-점검.md`](../notes/2026-08-08-행사-부하-용량-점검.md) §7.2.
...
| 날짜 | 변경 |
|---|---|
| 2026-05-31 | 백엔드 종합 설계 v1 |
| 2026-08-15 | 현행화 — 상단에 구현 괴리 표 추가. 스키마 3개(`rewards` 없음), S3 프리픽스 3종, **사진 영구 보관**(폐기 정책 삭제), 학생 토큰 12h, CORS 전체 개방 반영. §9 잡 큐와 카드 발급을 🔴 미구현으로 명시 |
