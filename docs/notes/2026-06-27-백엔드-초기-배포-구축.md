# 백엔드 초기 배포 구축 기록 — AWS Lightsail + Supabase

> **이 문서는 2026-06-27 시점의 기록이다. 현행 배포 절차가 아니다.**
>
> 지금 배포하려면 [`../guides/deployment.md`](../guides/deployment.md)를 본다.
>
> **이미 사실과 다른 부분**
> - 스토리지: 이 문서는 Supabase Storage(`photos`·`cards` 버킷)를 전제하지만,
>   현재는 **AWS S3 단일 비공개 버킷 + 프리픽스**(`uploads`/`ai-images`/`cards`)를 쓴다.
>   → `backend/.env.example`, [`../guides/backend-design.md`](../guides/backend-design.md) 참고.
> - 환경변수 목록(3장)에 이후 추가된 운영 필수 변수(`APP_ENV`·`FRONTEND_ORIGIN`·
>   `OPERATOR_PASSWORD` 등)가 빠져 있다.
>
> 인스턴스 생성·방화벽·Docker 설치·스왑·도메인 연결 등 **최초 1회 구축 절차**는
> 여전히 참고 가치가 있어 남긴다. 박제 문서이므로 내용을 갱신하지 않는다.

| 항목 | 값 |
|---|---|
| 작성일 | 2026-06-27 |
| 상태 | 기록 (당시 Draft — 실배포 전 합의/검증 단계) |
| 적용 대상 | `backend` (FastAPI) |
| 관련 문서 | [`../guides/backend-design.md`](../guides/backend-design.md) |
