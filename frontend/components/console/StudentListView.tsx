...
// 테스트 계정 관리는 관리자 전용 기능(백엔드도 CurrentAdminDep로 막음)이라
// 운영진에게는 탭 UI 자체를 렌더하지 않는다 — 운영진 세션에서는 tab이 항상 "members"다.
...
export function StudentListView() {
  ...
  // 테스트 계정 관리는 관리자 전용이라 운영진 세션에서는 아예 요청하지 않는다.
  ...
  // 백엔드도 관리자 토큰만 받으므로(CurrentAdminDep) 이 핸들러는 관리자 전용 UI에서만 연결한다.
  ...
  // 테스트 계정 일괄 삭제도 관리자 전용(백엔드 CurrentAdminDep) — 관리자 UI에서만 연결한다.
    ...
    >
      ...
      <main className="mx-auto max-w-6xl px-5 py-8 sm:px-6">
        ...
        {/* 탭 — 가입 회원과 테스트 계정은 서로 다른 집합이라 목록 자체를 분리한다.
            테스트 계정 관리는 관리자 전용 기능이라 운영진에게는 탭 UI 자체를 렌더하지 않는다. */}
        {canDelete && (
          <div
            role="tablist"
            aria-label="목록 종류"
        ...
        {/* 테스트 계정 관리 — 학생 로그인 화면으로 들어올 수 없는 kind='test' 계정을
            여기서만 발급·정리한다. 관리자 + 테스트 탭일 때만 노출한다(운영진은 탭 자체가 없어
            canDelete만으로도 충분하지만, hidden으로 탭 전환 시 DOM을 유지해 폼 입력을 보존한다). */}
        {canDelete && (
          <div
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-b bg-muted/50 hover:bg-muted/50">
                ...
                <TableHead className={canDelete ? undefined : "pr-5"}>
                  동의
                </TableHead>
                {/* 테스트 계정으로 진입하는 버튼 열 — 관리자 전용 기능이라 canDelete로
                    체크박스 열과 같은 방식으로 게이팅한다(운영진은 열 자체가 없다). */}
                {canDelete && <TableHead className="pr-5">관리</TableHead>}
              </TableRow>
            </TableHeader>
            <TableBody>
              {loading ? (
                ...
                <TableRow className="hover:bg-transparent">
                  <TableCell colSpan={canDelete ? 11 : 9} className="py-16">
                ...
                items.map((s) => (
                  ...
                  >
                    ...
                    <TableCell className="text-muted-foreground">
                      {/* 학교 없는 계정은 school이 빈 문자열이라 그대로 두면 칸이 비어
                          보인다 — 종류를 대신 표시한다(StudentDetailSidebar와 같은 규칙). */}
                      {s.kind === "test"
                        ? "테스트"
                        : s.kind === "guest"
                          ? "개인"
                    ...
                    <TableCell className={canDelete ? undefined : "pr-5"}>
                      <ConsentTag agreed={s.consent_privacy} />
                    </TableCell>
                    {/* "이 계정으로 테스트 시작" — 관리자 + 테스트 계정 행일 때만.
                        토큰 발급 자체가 관리자 전용이라 canDelete로 열을 게이팅한다. */}
                    {canDelete && (
                      <TableCell
                        className="pr-5"
                        onClick={(e) => e.stopPropagation()}
      ...
      {/* 테스트 계정 일괄 삭제 확인 — 발급·정리와 마찬가지로 관리자 전용이다. */}
      {canDelete && (
        <Dialog
